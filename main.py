...
class TelegramFileBot:
    ...
    def setup_web_routes(self):
        ...
        self.web_app.router.add_get('/status', self.handle_status_check)
    ...
    async def handle_web_root(self, request):
        ...
        <body>
            ...
            <p>Use /start in Telegram to begin.</p>
    ...
    async def handle_health_check(self, request):
        ...
        return web.json_response({"status": "ok", "service": "telegram-file-bot"})
    
    async def handle_status_check(self, request):
        """Handle status check request"""
        # Check bot status
        bot_connected = self.app.is_connected
        bot_info = await self.app.get_me() if bot_connected else None
        
        # Check Wasabi status
        wasabi_connected = await self.storage.test_connection()
        ...
        status_data = {
            "bot": {
                ...
                "name": bot_info.first_name if bot_info else None,
                "username": bot_info.username if bot_info else None
            },
            "wasabi": {
                "connected": wasabi_connected,
                "bucket": self.storage.bucket_name,
                "region": self.storage.region
            },
            "channel": {
                "connected": channel_connected,
                "id": self.storage_channel_id,
                "configured": bool(self.storage_channel_id)
            },
            "files": {
